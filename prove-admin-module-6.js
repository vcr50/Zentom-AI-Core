...
console.log("Starting Admin Module 6 progression proof...");
...
assert.equal(res6.data.score, 100);
...
assert.equal(passportUpdate.moduleId, "admin-6");
assert.equal(passportUpdate.skillId, "salesforce-reporting-dashboards");
assert.equal(passportUpdate.status, "verified");
assert.equal(passportUpdate.score, 100);
console.log("✓ Salesforce Reporting Dashboards skill verified!");

// 7. Verify that Admin Module 7 becomes the next unlock candidate
...
console.log("✓ Admin Module 7 is now eligible to unlock (Unlock Candidate)!");

console.log("\nAll Admin Module 6 progression proof assertions PASSED successfully!");
