...
console.log("Starting Admin Module 8 progression proof...");
...
// Check that Admin Module 8 unlocks
const unlock8 = getUnlockDecision({ studentAttempts, moduleId: "admin-7", tier });
assert.equal(unlock8.eligibleToUnlock, true);
console.log("✓ Admin Module 8 unlocked!");

// 8. Simulate passing Admin Module 8
console.log("Simulating Admin Module 8 lab completion...");
const res8 = await aiEngine.run("verify-lab", {
  moduleId: "admin-8",
  labId: "admin-8-lab-1",
  studentAnswers: {
    q1: "Student_Status_Update_Automation",
    q2: "Enrollment__c",
    q3: "Decision",
    q4: "Create Records",
    q5: "error handling and fail-safe operations"
  }
}, { userId: "founder-student-001", tier });

assert.equal(res8.ok, true);
assert.equal(res8.data.passed, true);
assert.equal(res8.data.score, 100);
...
assert.equal(passportUpdate.moduleId, "admin-8");
assert.equal(passportUpdate.skillId, "salesforce-flow-automation-intermediate");
assert.equal(passportUpdate.status, "verified");
assert.equal(passportUpdate.score, 100);
console.log("✓ Salesforce Flow Automation Intermediate skill verified!");

// 9. Verify that Admin Module 9 becomes the next unlock candidate
studentAttempts["admin-8:summary"] = { bestScore: res8.data.score };
const unlock9 = getUnlockDecision({ studentAttempts, moduleId: "admin-8", tier });
assert.equal(unlock9.eligibleToUnlock, true);
console.log("✓ Admin Module 9 is now eligible to unlock (Unlock Candidate)!");

console.log("\nAll Admin Module 8 progression proof assertions PASSED successfully!");
